...
```{r}
track <- read_excel('../01_rawdata/On-track and off-track countries.xlsx')
...
```{r, warning=FALSE}
demographic <- read_excel('../01_rawdata/WPP2022_GEN_F01_DEMOGRAPHIC_INDICATORS_COMPACT_REV1.xlsx', skip = 16,
                          sheet = 'Projections')
demographic %>% 
    filter(Year == 2022) %>% 
...
```{r, messages=FALSE}
indicators <- read_csv('../02_additionaldata/fusion_GLOBAL_DATAFLOW_UNICEF_1.0_.MNCH_ANC4+MNCH_SAB..csv', show_col_types = FALSE)
indicators %>% 
...
Interestingly, while skilled attendance at birth is nearly universal in on-track countries, antenatal care is only 73%: countries do achieve the on-track status while having a rather low antenatal care coverage, which makes you think that ANC4 is not the the strongest standalone predictor of new-born survival and achieving the on-track status.
